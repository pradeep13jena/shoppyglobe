  ...
  return (
    ...
    <div className='browseSection'>
      ...
      {/* .filter(book => book.title.toLowerCase().includes(valu.toLowerCase()) || book.author.toLowerCase().includes(valu.toLowerCase())) */}
        {arr1 ? arr1.filter(product => product.title.toLowerCase().includes(valu.toLowerCase())).map(product => <PItem id={product.id} key={product.id} discountPercentage={product.discountPercentage} src={product.thumbnail} title={product.title} brand={product.brand} price={product.price} rating={product.rating}/>) : 'Loading...'}
       </div>
    </div>
  </>
  )
